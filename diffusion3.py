...
def train_model(model, train_loader, optimizer, config, device,
    ...
    for epoch in range(config.EPOCHS):
        for images, _ in tqdm(train_loader, desc=f"Epoch {epoch+1}/{config.EPOCHS}"):
            ...
            xt = forward_diffusion(images, t, noise, sqrt_alphas_cumprod, sqrt_one_minus_alphas_cumprod)
...
def sample(model, config, device, betas, alphas, alphas_cumprod):
    ...
    x = torch.randn(num_samples, config.CHANNELS, config.IMG_SIZE, config.IMG_SIZE, device=device)
