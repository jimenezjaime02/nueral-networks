...
warnings.filterwarnings("ignore", message="Truncated File Read", module="PIL.TiffImagePlugin")
ImageFile.LOAD_TRUNCATED_IMAGES = True
torch.backends.cudnn.benchmark = True

# Hyperparameters
DATA_DIR = r'C:\Users\jimen\OneDrive\Desktop\neuralnetworks\kagglecatsanddogs_3367a\PetImages'
BATCH_SIZE = 32
NUM_WORKERS = 8
LR = 0.004   
WEIGHT_DECAY = 1e-4
...
def create_datasets_from_folder(data_dir, train_transform, test_transform):
    ...
    val_dataset = Subset(datasets.ImageFolder(data_dir, transform=test_transform), val_indices)
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, device, num_epochs, patience, grad_clip, warmup_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        print(f"Epoch [{epoch+1}/{num_epochs}] Train Loss: {train_loss:.4f}, Train Acc: {train_acc:.4f}, Val Loss: {val_loss:.4f}, LR: {current_lr:.6f}")
...
def main():
    ...
    print(f"Sample batch images shape: {sample_img.shape}, Sample batch labels shape: {sample_label.shape}")
    ...
    train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, device, NUM_EPOCHS, PATIENCE, GRAD_CLIP, WARMUP_EPOCHS)
